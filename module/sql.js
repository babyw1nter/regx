...
exports.queryTime = async (ip, time_count) => { // 查询同ip注册时间
	...
	let status = [false, false];
	let sql_ip_users = "SELECT `" + config.mysql.users.column.mySQLColumnId + "`, `" + config.mysql.users.column.mySQLColumnRegisterDate + "`, `" + config.mysql.users.column.mySQLColumnIp + "` FROM `" + config.mysql.users.tablename + "` WHERE `" + config.mysql.users.column.mySQLColumnIp + "` = '" + ip + "' ORDER BY `" + config.mysql.users.column.mySQLColumnId + "` DESC LIMIT 1;",
		sql_ip_authme = "SELECT `" + config.mysql.authme.column.mySQLColumnId + "`, `" + config.mysql.authme.column.mySQLColumnRegisterDate + "`, `" + config.mysql.authme.column.mySQLColumnIp + "` FROM `" + config.mysql.authme.tablename + "` WHERE `" + config.mysql.authme.column.mySQLColumnIp + "` = '" + ip + "' ORDER BY `" + config.mysql.authme.column.mySQLColumnId + "` DESC LIMIT 1;";
...
exports.insertReg = async (JSONdata, ip) => { // 插入数据表
	...
	let sql_authme = "INSERT INTO `" + config.mysql.authme.tablename + "` (`" + config.mysql.authme.column.mySQLColumnId + "`, `" + config.mysql.authme.column.mySQLColumnName + "`, `" + config.mysql.authme.column.mySQLRealName + "`, `" + config.mysql.authme.column.mySQLColumnPassword + "`, `" + config.mysql.authme.column.mySQLColumnRegisterIp + "`, `" + config.mysql.authme.column.mySQLColumnLastLogin + "`, `" + config.mysql.authme.column.mySQLlastlocX + "`, `" + config.mysql.authme.column.mySQLlastlocY + "`, `" + config.mysql.authme.column.mySQLlastlocZ + "`, `" + config.mysql.authme.column.mySQLlastlocWorld + "`, `" + config.mysql.authme.column.mySQLColumnRegisterDate + "`, `" + config.mysql.authme.column.mySQLColumnRegisterIp + "`, `" + config.mysql.authme.column.mySQLlastlocYaw + "`, `" + config.mysql.authme.column.mySQLlastlocPitch + "`, `" + config.mysql.authme.column.mySQLColumnEmail + "`, `" + config.mysql.authme.column.mySQLColumnLogged + "`, `" + config.mysql.authme.column.mySQLColumnHasSession + "`) VALUES (NULL, '" + username + "', '" + realname + "', '" + password_authme + "', NULL, NULL, '0', '0', '0', '" + config.mysql.authme.world + "', '" + date_timestamp + "', '" + ip + "', NULL, NULL, '" + email + "', '0', '0')";
	let query_users = new Promise((resolve, reject) => {
		mysql_pool.getConnection((error, connection) => {
			if (error) throw error;
			connection.query(sql_users, (err, res) => {
