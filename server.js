...
router.get("/api/get", async (req, res) => { // 处理get请求
	...
	switch(mod){
		case "check": // 实时检查
			if(!JSONdata.InputName || !JSONdata.InputVal){ // 非法参数
				res.sendStatus(403);
				return;
			}
		...
		case "sendecode": // 发送邮件验证码
			check.verifyCheck(config.verify.sendecode.AppID, config.verify.sendecode.AppSecretKey, JSONdata.ticket, JSONdata.randstr, api.getReqIp(req), res, async (err, verify_data) => {
				let rtn = {
					"mod": "sendecode",
					"msg": "",
					"email": JSONdata.e
...
express.post("/api/post", async (req, res) => { // post请求处理
	...
	if(!JSONdata || !JSONdata.id || !JSONdata.pwd || !JSONdata.e || !JSONdata.ecode || !JSONdata.ticket){ // 非法参数
		res.sendStatus(403);
		return;
	}
	switch(mod){
